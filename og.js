...
const pages = [
  ...
  { fileName: 'home', title: 'UniSurf', subtitle: 'Professionelles Web Hosting & Managed IT' },
  {
    fileName: 'services',
    title: 'Services',
    subtitle: 'Webentwicklung, Hosting & digitale Lösungen',
  },
  {
    fileName: 'entwicklung',
    title: 'Entwicklung',
    subtitle: 'Internetseiten, Webshops & Plattformen',
  },
...
try {
  ...
  console.warn(
    'Logo image not found at assets/images/unisurf-logo.png — continuing without embedded logo.'
  );
}
...
const NUNITO_FONT_PATH = path.join(
  process.cwd(),
  'assets',
  'fonts',
  'nunito-variablefontwght.woff2'
);
let nunitoFontFace = '';
try {
...
function makeSVG({ title, subtitle, brand, brandSubline, colors }) {
  ...
  const { fontSize: titleFontSize, lines: titleLines } = computeDynamicTitleLayout(
    title,
    availableWidth
  );
  ...
  const titleYStart = Math.max(
    padTop + titleFontSize + 40,
    Math.round(titleCenterRef - titleBlockHeight / 2)
  );
  const subtitleYStart = titleYStart + titleBlockHeight + Math.round(subtitleFontSize * 0.35);

  const titleTspans = titleLines
    .map(
      (l, i) => `<tspan x="${padLeft}" dy="${i === 0 ? 0 : titleLineGap}">${escapeXml(l)}</tspan>`
    )
    .join('');
  ...
  const subtitleTspans = subtitleLines
    .map(
      (l, i) =>
        `<tspan x="${padLeft}" dy="${i === 0 ? 0 : subtitleLineGap}">${escapeXml(l)}</tspan>`
    )
    .join('');
  ...
  lines.push('<defs>');
  lines.push(
    `<linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">${buildGradientStops(colors)}</linearGradient>`
  );
  lines.push(
    `<linearGradient id="glow" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="rgba(255,255,255,0.28)"/><stop offset="100%" stop-color="rgba(255,255,255,0)"/></linearGradient>`
  );
  lines.push(
  ...
  const outlineClass =
    '.outline{paint-order:stroke fill;stroke:#01324e;stroke-opacity:.40;stroke-width:1.3;stroke-linejoin:round;}';
  if (nunitoFontFace)
    lines.push(
      `<style><![CDATA[${nunitoFontFace} text, tspan { font-kerning:normal; } ${outlineClass}]]></style>`
    );
  ...
  lines.push(
    `<ellipse cx="${WIDTH * 0.72}" cy="${HEIGHT * 0.18}" rx="${WIDTH * 0.6}" ry="${HEIGHT * 0.6}" fill="url(#glow)" />`
  );

  lines.push(
    `<text x="${padLeft}" y="${padTop}" fill="#FFFFFF" class="outline" font-family="Nunito, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif" font-size="32" font-weight="800" letter-spacing="0.5" filter="url(#dropshadow)">${escapeXml(brand)}</text>`
  );
  if (brandSubline)
    lines.push(
  ...
  lines.push(
    `<text x="${padLeft}" y="${HEIGHT - 40}" fill="rgba(255,255,255,0.80)" class="outline" font-family="Nunito, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif" font-size="22" font-weight="700" filter="url(#dropshadow)">${escapeXml(FOOTER_DOMAIN)}</text>`
  );
...
async function generateOne({ fileName, title, subtitle }) {
  ...
  const { svg, logoBox } = makeSVG({
    title,
    subtitle,
    brand: BRAND,
    brandSubline: BRAND_SUBLINE,
    colors,
  });
  ...
  if (logoBuffer) {
    const resizedLogo = await sharp(logoBuffer)
      .resize(logoBox.width, logoBox.height, { fit: 'contain' })
      .toBuffer();
