...
## 🛑 Coding Guidelines & Constraints (Zero Hallucination)
...
- App: `http://localhost:8080` (or `${NGINX_PORT}`)
...
```
unisurf.de/
├── assets/                      # Frontend assets (TypeScript, SCSS)
│   ├── app.ts                   # Webpack entry point
│   ├── bootstrap.ts             # Stimulus controller bootstrap
│   ├── controllers/             # Stimulus controllers
│   ├── scripts/                 # UI behavior (TypeScript)
...
│   ├── symfony.md               # Symfony commands reference
│   └── database.md              # Database troubleshooting
│
├── .github/workflows/           # GitHub Actions CI/CD
│   ├── php.yml                  # PHP quality checks & tests
│   └── node.yml                 # Node/CSS/TypeScript checks
│
├── Dockerfile                   # Production Docker image
├── docker-compose.yaml          # Production compose
├── docker-compose.dev.yaml      # Dev overrides
├── docker-compose.mariadb.yml   # MariaDB service
├── docker-compose.mariadb.dev.yml # MariaDB dev overrides
│
├── composer.json                # PHP dependencies
├── composer.lock                # Locked PHP versions
├── package.json                 # Node dependencies
├── yarn.lock                    # Locked Node versions
├── webpack.config.js            # Webpack/Encore config
├── tsconfig.json                # TypeScript config
├── phpstan.dist.neon            # Static analysis config
├── phpunit.dist.xml             # Test runner config
├── .php-cs-fixer.dist.php       # Code formatting config
│
├── develop.sh                   # Local dev helper
├── deploy.sh                    # Production deploy helper
├── pipeline.sh                  # CI/CD pipeline runner
├── lint.sh                      # Run all linters
├── phpunit.sh                   # Run tests
├── php-cs-fixer.sh              # Run PHP formatter
│
...
- **Docker**: `docs/docker.md` — Setup, troubleshooting, container management
- **Symfony**: `docs/symfony.md` — Console commands, routes, configuration
- **Database**: `docs/database.md` — Migrations, backends, troubleshooting
